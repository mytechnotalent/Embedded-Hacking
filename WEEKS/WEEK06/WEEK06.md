...
> 📖 **Datasheet Reference:** The SIO block is at `0xd0000000` (Section 2.2, p. 31). GPIO_IN is at SIO offset `0x004` (Section 2.3.1.7, p. 55), returning the state of all GPIOs in a single read. The GPIO coprocessor (`mcrr p0`) provides single-cycle access to SIO GPIO registers (Section 3.7.5, p. 101–104).
