...
> 📖 **Datasheet Reference:** On the RP2350, the vector table location is configured via the VTOR (Vector Table Offset Register) item in the IMAGE_DEF block (Section 5.9.3.3, p. 425). The XIP flash base address is `0x10000000` (Section 2.2, p. 31). The Cortex-M33 vector table format follows the standard ARMv8-M layout with Initial SP at offset 0x00 and Reset at offset 0x04.
